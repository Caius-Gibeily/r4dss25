...
Read in `asd_stats-ET.xlsx` (containing x and y points of regard [x pos/y pos], the block number and participant ID), `block_movie-correspondence.xlsx` (which movie is viewed per block) and `block_session-sequence.xlsx` (which blocks were viewed per session per participant). You will need to load the package `readxl` (check `?read_excel` on tips for using the function). Ensure any metadata lines are skipped during import and, using a technique you learned in the previous class, clean the column names by handling spaces and uppercase characters. Try to remove the `DEX:`/`DEX-` prefix from each subject ID using `str_replace()`. The goal will be to create one super-tibble containing participant ID, block and session number, movie viewed and the coordinate data. You will need to perform two successive joins. Think about the keys you will require for each join to ensure unique rows are matched. Finally, use `geom_point` and a facet grid to plot each participants' points of regards per movie in separate plots.   
